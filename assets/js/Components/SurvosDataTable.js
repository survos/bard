...
export default class SurvosDataTable {
    ...
    dataTableParamsToApiPlatformParams(params) {
        ...
        // this is the global search, good for ES, Or it could be the primary text field, like title, defined in the table, search-field
        ...
        console.log('params from dtSearch to ApiData', params, apiData);
    ...
    apiRequest(options, apiData) {
        ...
        this.debug && console.warn(options);
        ...
        */
        ...
        return function (params, callback, settings) {
            ...
            console.error(`DataTables is requesting ${params.length} starting at ${params.start}`);
            ...
            var jqxhr = $.ajax(options)
                ...
                .done(function (hydraData, textStatus, jqXHR) {
                    ...
                    if (next) {
                        ...
                        {
                            console.warn('next page has been fetched.');

                            d = d.concat(json['hydra:member']);
                            this.debug && console.warn(d.map( obj => obj.id ));

                            d = d.slice(params.start - first, (params.start - first) + params.length);
                            let itemsReturned = d.length;
                            console.warn(d);
                            console.error(`  ${itemsReturned} (of ${total}) returned, page ${apiOptions.page}, ${apiOptions.itemsPerPage}/page first: ${first} :`, d);
                            if (this.debug) {
                            }

                            console.log(`2-page callback with ${total} records`);
                            console.warn(callback, params, d, total, itemsReturned);
                            callback({
                                draw: params.draw,
                                data: d,
                                recordsTotal: total,
    ...
    render() {
        ...
        const dtOptions = {
            ...
            displayLength: 25, // not sure how to adjust the 'length' sent to the server
            scroller: {
                // rowHeight: 20,
                displayBuffer: 18,
        ...
        let $table = this.el.DataTable(dtOptions);
        console.log( 'DataTable Init requested at: '+new Date().getTime() );
        $table.on( 'draw', function () {
            this.debug && console.warn(this.el.attr('id') + ' drawn!');
